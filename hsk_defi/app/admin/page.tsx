        ...
        {invoices.map((inv, i) => (
          ...
          >
            <div>
              ...
              <div className="text-sm text-muted-foreground">ID: {inv.id}</div>
              <div className="text-sm">현재 상태: {inv.status}</div>
            </div>

            <div className="flex items-center gap-4">
              <Select
                defaultValue={inv.status?.toString()}
                onValueChange={(val) => handleStatusChange(BigInt(inv.id), Number(val))}
              >
